...
/**
 * The class represents an inventory of items that each have a quantity and a critical date. 
 * The list explicitly keeps track of both the quantity and critical date for every item. 
 * No item in the inventory ever has a null date or non-positive quantity. In the inventory, 
 * items are identified by both the item object and their date and .equals is
 * used to compare items and dates. (Items that are the same but have different critical 
 * dates are kept separately in the inventory.)
 */
